...
2. **Secret scanning = gitleaks** (`gitleaks/gitleaks-action@v2`) on every PR + push to main. A `.gitleaks.toml` extends the default rules with an allowlist for the test password and `.env.example` placeholders (no real secrets exist, but this prevents future leaks). Fails CI on any finding.
